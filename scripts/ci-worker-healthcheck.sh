...
# The long-running worker needs its runtime values; curl needs neither runtime
# secrets nor the build-only private provenance checkout path. Remove them
# in-shell before any external process is created, then replace this shell with
# the readiness probe.
